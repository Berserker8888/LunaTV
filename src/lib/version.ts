...
export const CHANGELOGS: ChangelogItem[] = [
  {
    ...
    date: '2026-09-03',
    content: `
- 播放：跳過片尾會正確鎖定進度並取消連播倒數，不再一次跳兩集。
- 播放：換源到不同集數時不再用上一集的秒數去恢復；切集會顯示載入蒙層。
- 播放：片頭片尾也能快進／快退；快捷鍵說明補上子母畫面與 Esc。
- 搜尋：虛擬列表改跟視窗捲動對齊，往下捲不再出現空白列。
- 搜尋：換關鍵字會清掉舊篩選；篩選後沒結果可一鍵清除；回頂按鈕會出現。
- 搜尋：空白輸入框對焦會顯示最近搜尋與熱門關鍵字。
- 登入：封禁、限流會顯示真正原因，不再一律寫「密碼錯誤」。
- 搜尋：漢字中間的「の」會轉成「的」再送陸源（進撃の巨人→进击的巨人），不再因假名整段放棄。
- 搜尋：不再把 は／と 從假名單詞裡刪掉或改成「和」；動漫副標 ～、× 會拆開再搜。
- 搜尋：「最終季／終章」不再硬當成第 4 季，避免兩季或三季完結被濾光。
- 安全：收藏／觀看紀錄等寫入改查同源 Origin；管理員「權限不足」改回 403。
- 安全：空白 PASSWORD 不再當成有效密鑰；代理分片一律限大小；轉發標頭需 TRUST_PROXY。
    `.trim(),
  },
