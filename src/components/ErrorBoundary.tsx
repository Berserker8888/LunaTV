    ...
    if (this.state.error) {
      ...
      return (
        <main className='flex min-h-screen items-center justify-center bg-deep p-6 text-zinc-100'>
          <section className='w-full max-w-md rounded-lg border border-zinc-800 bg-zinc-900 p-6 text-center shadow-sm'>
            <h1 className='text-xl font-semibold'>頁面發生錯誤</h1>
            <p className='mt-3 text-sm text-zinc-400'>
              請重新整理頁面，或返回首頁後再試一次。
            </p>
            {process.env.NODE_ENV === 'development' && (
              <pre className='mt-4 max-h-40 overflow-auto rounded-md bg-zinc-950 p-3 text-left text-xs text-zinc-300'>
            ...
            <div className='mt-5 flex justify-center gap-3'>
              ...
              <Link
                href='/'
                className='rounded-md border border-zinc-700 px-4 py-2 text-sm font-medium text-zinc-200 transition-colors hover:bg-zinc-800'
              >
                返回首頁
              </Link>
            </div>
